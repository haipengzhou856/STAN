...
## NOTE: Ignore the 'dark', which is just an attempt in my exp
## more detail see our another work
## shadow refering detection via language guided, proposed by hongqiu
## I'm lazy to replace it :p
...
class ViSha_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        clip_list, label_list, dark_list = self.joint_transform(clip_list, label_list, dark_list)
        image_torch = torch.stack(clip_list)
        label_torch = torch.stack(label_list)
        dark_torch = torch.stack(dark_list)
        threshold = 0.05
        dark_torch = (dark_torch > threshold).to(torch.long)

        return {"image": image_torch, "label": label_torch, "dark": dark_torch,
